CREATE DATABASE PROJETO_LT;
USE PROJETO_LT;

CREATE TABLE LOGIN(
    LOGIN_ID INTEGER NOT NULL AUTO_INCREMENT,
    CONSTRAINT LOGIN_ID_PK PRIMARY KEY (LOGIN_ID)
);

CREATE TABLE SUPERVISOR(

  SUPER_ID INTEGER NOT NULL AUTO_INCREMENT,
  SUPER_NOME VARCHAR(45) NOT NULL,
  SUPER_EMAIL VARCHAR(255) NOT NULL,
  SUPER_SENHA VARCHAR(70) NOT NULL,
  CONSTRAINT SUPER_ID_PK PRIMARY KEY (SUPER_ID),
  CONSTRAINT SUPER_ID_FK FOREIGN KEY (SUPER_ID) REFERENCES LOGIN(LOGIN_ID)

);

CREATE TABLE USUARIO(

  USER_ID INTEGER NOT NULL AUTO_INCREMENT,
  USER_NOME VARCHAR(45) NOT NULL,
  USER_EMAIL VARCHAR(255) NOT NULL,
  USER_NUM_CELULAR VARCHAR(13),
  USER_SENHA VARCHAR(70) NOT NULL,
  CONSTRAINT USER_ID_PK PRIMARY KEY (USER_ID),
  CONSTRAINT USER_ID_FK FOREIGN KEY (USER_ID) REFERENCES LOGIN(LOGIN_ID)

);

